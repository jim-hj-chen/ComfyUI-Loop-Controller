...
**机制**
- 节点声明 `INPUT_IS_LIST = True`，一次接收上游的完整 LIST，避免 ComfyUI 在同一个 prompt 内隐式批处理所有元素
- `index` 会按 ComfyUI 的列表输入规则解包为当前轮次的单个整数
- 多行字符串会自动按 `\n` 拆分并去掉空行
- `list/tuple` 保持完整列表语义，每个 prompt 只返回当前索引对应的一个元素
- 非列表输入会转为单元素列表兜底
...
>
> 例如 5 条数据且每条耗时约 5 秒时，任务历史应显示 5 个各约 5 秒的独立任务；
> 不应再显示为 1 个约 25 秒的任务。
...
- `List Item Extractor` 会完整接收 LIST，阻断 ComfyUI 的隐式 list fan-out；同一个 prompt 只提取并处理一个元素。
...
- 失败轮次不会执行到 `Loop Trigger`，因此不会自动提交下一轮，循环会停在当前索引。
- 之前轮次属于已经完成的独立任务；本插件不会清理或回滚它们的输出文件。
- 修复失败原因后，将 `Loop Start.start_index` 设置为失败轮次的索引，再手动点击 Queue，即可从该项重新开始。
- 例如索引 `0`、`1` 已成功，索引 `2` 失败，则设置 `start_index = 2` 后重新 Queue。
